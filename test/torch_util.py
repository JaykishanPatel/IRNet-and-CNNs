import os, random, glob, numpy as np, torch, imageio, cv2
...
def hdrreadcv2( fname, ftag=None ):
    if not ftag is None:
        fname = fname[:-4] + ftag + fname[-4:]
    im = np.array( cv2.imread(fname), dtype = np.float32)
    return im
...
def gen( directory='/hdd/geometric', batchsize=10, tensor=True, device=None ):
    ...
    for i in range(0,len(flist),batchsize):
        ...
        for j in range(batchsize):
            fname = flist[i+j]
            lumbatch[j,] = flip( hdrreadcv2( fname ) )
            refbatch[j,] = flip( hdrreadcv2( fname, '_ref' ) )
            # nrmbatch[j,] = flip( hdrread( fname, '_nrm' ) )
            filebatch.append(fname)
        ...
        yield lumbatch, refbatch, filebatch # , nrmbatch
